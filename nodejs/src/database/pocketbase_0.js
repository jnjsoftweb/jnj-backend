...
class PocketBaseDB {
  ...
  async upsert(collectionName, dataArray, uniqueField) {
    const results = [];
    for (const data of dataArray) {
      try {
        const result = await this.upsertOne(collectionName, data, uniqueField);
        results.push(result);
      } catch (error) {
        console.error(`Failed to upsert record with ${uniqueField}: ${data[uniqueField]}`, error);
        results.push(null);
      }
    }
    return results;
